...
DisableMSDefender: {Name: "Disable Microsoft Defender", Desc: "To disable Microsoft Defender Antivirus you need to do 2 steps:`n`n- Check Disable Microsoft Defender.`n- Enter Safe Mode and check it again.`n`nTo turn Microsoft Defender back on, you just need to do the opposite.`n`n(If you enter Safe Mode using this program, you need to use this program to exit Safe Mode`n  or you can use the command line 'bcdedit /deletevalue `"{current}`" safeboot')"},
...
Message_WaitDlg: "Please wait...",
Message_ClearStartMenu_Done: "Clear StartMenu Done!",
Message_DisableMSDefender_GoSafeMode1: "You need go to Safe Mode and Check it again.`nWould you like go to Safe Mode?",
Message_DisableMSDefender_GoSafeMode0: "You need go to Safe Mode and Uncheck it again.`nWould you like go to Safe Mode?",
Message_DisableMSDefender_ExitSafeMode: "Would you like exit Safe Mode?"
},
zh_cn: {
Name: "中文（简体）",
Translator: "Jvcon",
...
DisableMSDefender: {Name: "禁用Microsoft Defender", Desc: "要禁用Microsoft Defender防病毒，你需要做两步：`n`n- 勾选禁用Microsoft Defender。`n- 进入安全模式并再次检查。`n`n要打开Microsoft Defender，你只需要做相反的操作。`n`n（如果你使用此程序进入安全模式，你需要使用此程序退出安全模式`n  或者你可以使用命令行 'bcdedit /deletevalue `"{current}`" safeboot'）"},
